...
mysqlconnection = dbConnect(MySQL(), dbname = "CMRC_DB_before",user = 'kaonthu', password = 'asd123',host = '192.168.1.41')
...
FROM CMRC_DB_before.new_CMRC_db_20976_desc_20220127;
")
...
barplot(table(SCORE$c2_Latency))
...
barplot(table(SCORE$c3_Duration))
...
barplot(table(SCORE$c4_Efficiency))
...
barplot(table(SCORE$c5_Disturbance))
...
barplot(table(SCORE$c6_Use_Medicatin))
...
barplot(table(SCORE$c7_Daytime_dysfunction))
##################################################
...
Anxiety=Anxiety,Depression=Depression,HAD=HAD,TDQ=TDQ,AD8=AD8,PSQI=PSQI,AUDIT=AUDIT,R=R)

dbWriteTable(mysqlconnection,"new_CMRC_db_20976_Scale_desc_20220127_R",out,append=TRUE)
