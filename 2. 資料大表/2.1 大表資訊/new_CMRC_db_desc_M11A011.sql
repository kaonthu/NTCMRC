UPDATE NTCMRC.new_CMRC_db
SET HS_CRP = replace(HS_CRP,"<","");
UPDATE NTCMRC.new_CMRC_db
SET HS_CRP = replace(HS_CRP," ","");

UPDATE NTCMRC.new_CMRC_db as a 
INNER JOIN 
NTCMRC.CMRC_all_persontimes as b
ON a.sid = b.P_dn 
...
INSERT INTO NTCMRC.new_CMRC_db
...
FROM NTCMRC.CMRC_all_persontimes
WHERE P_dn regexp '^M11';

SELECT sid,P_Number,n_1,CMRC_id,New_CMRC_id,p_id,sex,birth,age,data_exam,data_year,year_come
FROM NTCMRC.new_CMRC_db;

CREATE TABLE CMRC_DB_kaonthu_2.CMRC_M11A011 (
...
'D:/Leeyu/JupyterWorkplace/20211229_kaonthu_report/檢驗值匯入Mysql/M11A009_report_檢驗值.csv'
INTO TABLE CMRC_DB_kaonthu_2.CMRC_M11A011
CHARACTER SET utf8 FIELDS 
TERMINATED BY ','  
LINES TERMINATED BY '\n' 
IGNORE 1 LINES;

#ALTER TABLE `NTCMRC`.`new_CMRC_db_20963_20220310` 
...
UPDATE NTCMRC.new_CMRC_db as t1
INNER JOIN CMRC_DB_kaonthu_2.CMRC_M11A011 as t2
ON t1.p_id = t2.`病歷號`
SET
...
UPDATE NTCMRC.new_CMRC_db_desc
...
ALTER TABLE `NTCMRC`.`new_CMRC_db` 
CHANGE COLUMN `SP_Gravity` `SP_Gravity` VARCHAR(30) NULL DEFAULT NULL ;

INSERT INTO NTCMRC.new_CMRC_db_desc 
SELECT * FROM NTCMRC.new_CMRC_db
ORDER BY data_exam DESC,sid DESC; #反過來排序alter 要先truncate
...
SELECT sid, n_1, p_id, sex, birth, age, height_y, weight_y, waist_y, systolic_y, diastolic_y, heartbeat_y, arm_circumference, calf_circumference, Thigh_circumference, Glucose_AC_y, Triglyceride_y, HDL_C_y, WBCb, RBCb,
    ...
    FROM NTCMRC.new_CMRC_db_desc
	WHERE sid regexp 'M11A' ;
    
    SELECT sid, n_1, p_id, sex, birth, Anti_HCV_x,Leukocyte, Nitrite, Protein,
    Glucose, Ketone, Urobilinogen, Bilirubin, octBlood, RBCu, WBCu, Squamus_Epi, UreaNU, MicroALB_U, Creatinine_U, TP_U, Alb_Cre_ratio, TP_U_Cre_ratio, HOMA_IR, Insulin_GLU, Adiponectin, Leptin, TotalVitaminD, Vitamin_B12, Lymphocyte, Monocyte,
    Eosinophil, Basophil, Segment, year_come
    FROM NTCMRC.new_CMRC_db_desc
